...
An **idea roll** is an optional rule. If you're not sure how to approach a situation, you can roll an **Intuit** check. If you pass, the narrator can give you some helpful suggestions.

You can handle failure in a few ways:

- withhold suggestions
- give potentially unhelpful suggestions that may accomplish the goal, but with some unwanted side-effect
- give less suggestions than you would with a success
...
- If attacking with **Shoot**, the other character cannot be in immediate range, and cannot be at a distance further than your **Sense** score (if your **Sense** is 5, they must be 5 or fewer zones away)
...
Characters may always move from one zone to another, as long as there's nothing that logically impedes them. For example, a wall between two zones would keep a character from moving freely between them. For something like a huge chasm, they might require a successful **Jump** check.

Zones may also have a vertical component, like an upper balcony, a tall tree with high-up branches to perch on, or a higher space where someone is flying with wings or Wind art. The same considerations apply; if there's no feasable way for someone to fly or otherwise elevate themselves, they wouldn't be able to enter a zone above them.
