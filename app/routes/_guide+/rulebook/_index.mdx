...
Characters in Aspects of Nature are often humans with traits that follow some sort of creature or fantasy lineage. Think cat-like humans with cat ears and long thin tails; part-dragons with horns, glowing eyes, and patches of scales on their bodies; part-lizards with scales and skin fading to green; or anything else you can think of.
...
1. Your character's [level](#levels-and-progression), which determines how many stat points you get. Your group can start at any level you like.
1. Assign each [attribute](#attributes) a number from 1 to 6. The sum depends on your [starting level](#levels-and-progression).
1. Assign each [aspect](#aspects) a number from 0 to 6. The sum depends on your [starting level](#levels-and-progression).
...
Attributes represent your character's core strengths of various capabilities or domains. Each player character should have a score for each attribute from 1 to 6, adding to a sum depending on your [level](#levels-and-progression).
...
You may choose to rest for a number of in-game hours, recovering one hit and fatigue per hour.
...
Every character has a base score for each Aspect from 0 to 6, adding to a sum depending on your [level](#levels-and-progression). Then add the character's score for the aspect's linked attribute to get the aspect's total score.
...
Your character's **attuned** aspects are those with a base score of one or more. For example: if you assign 4 points to darkness and 2 points to water, your attuned aspects are darkness and water. Fire, light, and wind are unattuned.

<AspectList />

### Aspect Art

The manipulation of aspects is called **aspect art**, of which there are two categories: **minor**, and **major**.

Minor arts are more subtle, low-risk, and low-impact. Minor arts are the sorts of arts used in normal day-to-day life. Examples include:
...
Major aspect arts are more powerful and cost more to perform. They can be used to great effect, whether to help a friend or to cause a great deal of damage. Major aspect arts require an [aspect check](#aspect-checks). Examples include:
...
For attuned aspects, minor arts are free, while major arts cost **1 fatigue**.

For unattuned aspects, minor arts cost **1 fatigue**, while major arts cost **2 fatigue**, while adding a risk die to the roll.

## Levels & Progression

<Callout>This has not yet been implemented on character sheets.</Callout>

Your character's level represents their experience and development. All character levels in a group should be the same, and you can start at any level you like. Your level increases on each **milestone**, such as a pivotal plot point, major accomplishment, or notable checkpoint.
...
Action rolls use a roll-under system. For a roll to succeed, it must be **equal or lower than the target value**, depending on the type of roll being made.

**Only use action rolls when the action has some chance of failure.** Otherwise, the narrator should allow the action without a roll.
...
An aspect check is a special kind of skill check to perform a major aspect art. The roll is made with **a d12 die** instead of a d6, and the target is the character's score in the corresponding aspect.

For example: you want to use _Shield_, which is a water aspect art. If your total water score is 7 (Base 3 + Sense 4), you would need to roll a 7 or lower to succeed.
...
For example: a successful check for _Inspire_ will add 2 power dice to a target's next roll. On failure, it will give them 1 power die instead.

Below is a list of built-in aspect arts. Think of them as presets; you can still perform other "freeform" aspect arts not listed here.
...
<Callout>
	You may have noticed that attributes and aspects set to their max level of 6
	and 12 would effectively make them always pass, but in this case, you can
	still fail a roll through risk dice, explained in the next section.
</Callout>
...
- Power dice **subtract** from the roll, narratively making the roll easier.
- Risk dice **add** to the roll, narratively making it harder.
